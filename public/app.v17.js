...
VaultChat.renderChatPanel = function(container) {
  ...
  container.innerHTML =
    '<div class="chat-panel">' +
      '<div class="chat-header"><div id="context-badges"></div></div>' +
...
VaultChat.sendChatMessage = function(container) {
  ...
  // Auto-search knowledge base if no note context
  if (V.state.noteContext.length === 0 && V.getToken()) {
    ...
    V.searchNotes(text).then(function(searchData) {
      var promises = [];
      var results = searchData.results.slice(0, 3);
      for (var i = 0; i < results.length; i++) {
        promises.push(V.fetchNote(results[i].path));
      }
      ...
      // Search failed, chat without context
      doStreamChat();
    });
  } else {
    doStreamChat();
